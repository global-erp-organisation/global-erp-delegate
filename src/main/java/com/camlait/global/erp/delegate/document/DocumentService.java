...
public class DocumentService implements IDocumentService {
	...
	public LigneDeDocument ajouterLigneDocument(@NonNull LigneDeDocument ligne) {
		if (!ligne.getProduit().isAvailable(ligne.getDocument().getMagasin(), ligne.getQuantiteLigne())) {
			throw new GlobalErpServiceException(unavailableProductMessage(ligne));
		}
	...
	public double obtenirPrixUnitaire(@NonNull Long partenaireId, @NonNull Long produitId) {
		...
		if (p != null) {
			return p.getTarifications().stream()
					.filter(t -> t.getTarif().getTarifId().equals(c.getTarif().getTarifId()))
					.collect(Collectors.toList()).get(0).getPrixUnitaire();
		}
		return 0;
	}
	...
	public double obtenirPrixUnitaireMarge(@NonNull Long partenaireId, @NonNull Long produitId) {
		...
		if (p != null) {
			return p.getTarifications().stream()
					.filter(t -> t.getTarif().getTarifId().equals(c.getTarif().getTarifId()))
					.collect(Collectors.toList()).get(0).getPrixUnitaireMarge();
		}
		return 0;
	}
}
